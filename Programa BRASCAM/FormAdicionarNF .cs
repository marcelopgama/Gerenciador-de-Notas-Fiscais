...
{
    public partial class FormAdicionarNF : Form
    {
        bool arrastar = false;
        int posiçãoX, posiçãoY;       
        public object Frete = 0;
        public int CodigoDaNota = 0;
        ...
        private void button4_Click(object sender, EventArgs e)
        {
                ...
                & String.IsNullOrEmpty(textBox6.Text)==false & String.IsNullOrEmpty(comboBox2.Text)==false)
            {
                try
                {
                    NumeroDaNota = Convert.ToInt32(textBox6.Text);
                }
                catch
                {
                    MessageBox.Show("O número da nota deve conter apenas números.");
                    return;
                }
                ...
                try
                {
                    if (Cliente != null | Cliente != "")
                    {
                        ...
                        SqlCommand command = new SqlCommand("SELECT CodDoCliente FROM BRASCAMnotas WHERE " +
                            "(DataDaNota>='" + dataFilter + "' AND NumDaNota=" +
                ...
                Tipo = comboBox1.SelectedIndex + 1;
                Status = comboBox2.SelectedIndex + 1;
        ...
        void ProcurarCliente()
        {
            if (textBox6.Text != String.Empty)
            {
                try
                {
                    ...
                    SqlCommand command = new SqlCommand("SELECT Cliente, NumDoPedido, CodigoDaNota, Frete, DataDaNota FROM BRASCAMnotas WHERE " +
                        "(DataDaNota>='" + dataFilter + "' AND NumDaNota=" +
                        textBox6.Text + ") ", connection);
                    ...
                    while (reader.Read())
                    {
                        ...
                        try
                        {
                            NumeroDoPedido = reader.GetInt32(1);
                        }
                        catch { }
                        try
                        {
